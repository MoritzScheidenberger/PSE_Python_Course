...
**The Job Ladder is Important**

*(Job-to-Job transitions (henceforth also EE transitions) are at the heart of the job ladder. While already employed, workers eventually encounter job offers at other firms that constitute an improvement in their career. In other words, by transitioning from one employer to the other, workers typically climb another rung on the job ladder, towards their ultimate career-goal job).*

1. Half or all hires are EEs i.e. EE flows = EU flows in terms of size
2. EE transitions collapse in recessions
3. On-the-job search is very effective (Faberman et al. 2022)
4. EE flows affect income, consumption, productivity, inflation (See BKMS 2024, MPV 2023)

Additional Notes:

- Job security matters a lot for people staying in their job (German data I think)

*Story:*

- The response of replacement hiring to uncertainty matters because it could indicate that the job ladder collapses temporarily
- This goes well with what Gottfries said about that discount factor shocks matter less for labor markets because of the high turn-over / people don't stay in jobs that long so their discounting matters less. However, replacement hiring could affect the age of jobs / how quickly the hiring chain breaks down


**Replacement hiring as *pendant* to capital utilization**

*Story building:*

- Frame it such that job creation is what builds a stock of jobs over time and replacement hiring determines how quickly this stock depreciates & how much it is utilized
- These empty positions in response to uncertainty come from the waiting
-


**Why does it differ from Job Creation**
- Why do we care where vacancies come from?



## How to Measure Replacement Hiring
While there exists data on the number of replacement hires, firms' decisions to rehire for workers that leave cannot be inferred directly. Since the number of replacement hires is capped by the number of workers quitting their positions, any change in worker quits mechanically reduces the number of replacement hires. Thus, to estimate the number of quits that remain unreplaced, one has to control for any movement in replacement hiring that is driven by a change in the number of workers quitting their jobs and leaving behind a viable position.
...
2. Infer the number of viable positions left behind from data on job-to-job transitions. The argument goes as follows:
...
Think about if I can do anything about job destruction in excess of quits and layoffs etc.
...
fun_plot_single_ts(
    ...
    xlabel="Date",
    ylabel="Share of Replacement Hires",
    recession_shading=True,
    scale_factor=1
)


# Call function with HirA as the line series
fun_plot_two_bars(
...
# Series of EE transitions per replacement hires: using the rates -> CPS EEs
...
::: {.grid}
::: {.g-col-6}
![](main_quarto_files/plot_series/plot_series-output-1.svg){.lightbox}
:::
::: {.g-col-6}
![](main_quarto_files/plot_series/plot_series-output-2.svg){.lightbox}
:::
:::
