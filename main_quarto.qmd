...
Note that the data on EE transitions as well as some key macroeconomic time series are obtained directly through the FRED, the API of the St. Louis Federal Reserve.
...
taggr_methods = {
    "sum": ["ts_jopen_nfarm_lvl_m", "ts_jopen_tpriv_lvl_m", "ts_quit_nfarm_lvl_m", "ts_quit_tpriv_lvl_m",
             ...
             "ts_osep_nfarm_lvl_m", "ts_osep_tpriv_lvl_m", "ts_tsep_nfarm_lvl_m", "ts_tsep_tpriv_lvl_m", "ts_ee_prob_m"],
    "mean": ["ts_jopen_nfarm_rate_m", "ts_jopen_tpriv_rate_m", "ts_quit_nfarm_rate_m", "ts_quit_tpriv_rate_m",
              ...
              "ts_cpi_m", "ts_unrate_m", "ts_fedfunds_m"]
}

# Getting the dates right seems difficult
for method, series_list in taggr_methods.items():
    for series in series_list:
        ...
        # Adjust dates to the start of the quarter
        df_q.index = df_q.index.to_period("Q").to_timestamp(how="S")  # 'S' for Start of Period

        # Drop quarters with missing data
        df_q = df_q.dropna()
...
df_qwi_Emp = df_qwi_filtered[["date", "Emp"]].copy()
...
df_qwi_Emp = df_qwi_Emp[(df_qwi_Emp["date"] >= start_date) & (df_qwi_Emp["date"] <= end_date)]
...
Next, we can go on to construct the series of job-to-job (EE) transitions relative to replacement hires with which we intend to track firms' replacement behavior.
Since the data on EE transitions (constructed by Postel-Vinay and Moscarini) is based on the CPS (which samples the whole population), we need align it with the fact that the QWI only covers employees in the private sector.
For now, this is done by multiplying the EE transition rate (i.e. the percentage of employees who switch jobs) with the total employment in the private sector (QWI).
By doing so, we implicitly assume that the transition rates do not differ significantly between the sample of all private employees and the whole population.
Given that private employment dominates total employment, this assumption seems reasonable.

```{python}
#| label: plot_ee_repl
#| fig-cap: ""
#| fig-subcap:
#|   - "The measure of replaceable positions is constructed by dividing the EE transitions by replacement hires. It shows the number of job-to-job transitions per replacement hire. Note that the data for EE transitions is based on CPS data while that of replacement hires is based on census data for all private firms. Adjustments were made to make these series comparable."
#|   - "In essence, uncertainty is measured as the unforecastable component at any point in time. When it rises, it can be interpreted as the future becoming more uncertain / harder to forecast. For details, see the AER paper by Jurado et al. (2015)."
#| layout-ncol: 2

## Construct EE transitions relative to replacement hires

# need to restrict Emp version to 1995-10-1 to 2023-1-1 first because EE data starts in 1995
df_qwi_Emp_short = df_qwi_Emp[(df_qwi_Emp["date"].copy() >= "1995-10-01") & (df_qwi_Emp["date"].copy() <= end_date)]

# These indices in python are weird to work with :/ Now it finally works.
ts_EE_in_qwiEmp = ts_ee_prob_q.copy()
ts_EE_in_qwiEmp["Value"] = df_qwi_Emp_short["Emp"].values * (ts_ee_prob_q["Value"].values / 100)

# Shorten HirAEndRepl first (tedious...)
ts_HirAEndRepl_short = df_qwi_ReplHires[(df_qwi_ReplHires["date"] >= "1995-10-01") & (df_qwi_ReplHires["date"] <= end_date)]

ts_replaceablePositions = ts_EE_in_qwiEmp.copy()
ts_replaceablePositions["Value"] = 100*ts_EE_in_qwiEmp["Value"].values / ts_HirAEndRepl_short["HirAEndRepl"].values

# shorten ts_replaceablePositions because of outliers in last period
ts_replaceablePositions = ts_replaceablePositions[ts_replaceablePositions["Date"] < "2022-10-01"]

fun_plot_single_ts(
    df=ts_replaceablePositions,
    title="Measure of Replaceable Positions",
    xlabel="Date",
    ylabel="Job-to-Job Transitions per Replacement Hire",
    recession_shading=True,
    scale_factor=1
)

# Let's add the measure for uncertainty
fun_plot_single_ts(
    df=ts_jln_uncertainty_q,
    title="Uncertainty Measure",
    xlabel="Date",
    ylabel="Uncertainty (by Jurado et al.)",
    recession_shading=True,
    scale_factor=1
)
...
From these figures, we do see a similarity in the cyclicality. The number of unreplaced quits (i.e. share of EE transitions per replacement hire) rises spikes when uncertainty spikes and otherwise returns to a rather stable value of 0.9.
Moreover, the value of 0.9 seems reasonable. In theory, one would expect a value of 1 in normal times i.e. when all quits are replaced. However, factors such as quits into non-employment (retirement etc.) imply that there are more positions to be replaced than EE transitions.


```{python}
#| label: VAR_estimation
#| echo: false
#| warning: false
#| fig-cap: "IRFs to a one std. shock to uncertainty. Identification  relies on cholesky ordering is identical to that of the subfigures. Ordering follows the literature."
...
# Variable list:
# - Uncertainty: ts_jln_uncertainty_q
# - Replaceable Positions: ts_replaceablePositions
# - Unemployment Rate: ts_unrate_q
# - CPI: ts_cpi_q (take log)
# - Interest Rate: ts_fedfunds_q

ts_CPIlog_q = ts_cpi_q.copy()
ts_CPIlog_q["Value"] = np.log(ts_CPIlog_q["Value"])

# Align the sample
start_date = "1995-10-01"
end_date = "2022-10-01"

ts_jln_uncertainty_q_var = ts_jln_uncertainty_q[(ts_jln_uncertainty_q["Date"] >= start_date) & (ts_jln_uncertainty_q["Date"] <= end_date)].rename(columns={"Value": "Uncertainty"})
ts_replaceablePositions_var = ts_replaceablePositions[(ts_replaceablePositions["Date"] >= start_date) & (ts_replaceablePositions["Date"] <= end_date)].rename(columns={"Value": "Unreplaced_Quits"})
ts_unrate_q_var = ts_unrate_q[(ts_unrate_q["Date"] >= start_date) & (ts_unrate_q["Date"] <= end_date)].rename(columns={"Value": "Unemployment_Rate"})
ts_CPIlog_var = ts_CPIlog_q[(ts_CPIlog_q["Date"] >= start_date) & (ts_CPIlog_q["Date"] <= end_date)].rename(columns={"Value": "Log_CPI"})
ts_fedfunds_var = ts_fedfunds_q[(ts_fedfunds_q["Date"] >= start_date) & (ts_fedfunds_q["Date"] <= end_date)].rename(columns={"Value": "Interest_Rate"})

# List of DataFrames containing time series data
dataframes = [ts_jln_uncertainty_q_var, ts_replaceablePositions_var, ts_unrate_q_var, ts_CPIlog_var, ts_fedfunds_var]
...
# Fit the VAR model
model = VAR(merged_df)
results = model.fit(maxlags=8, ic='aic', trend='c')


# Select shock
shock_variable = merged_df.columns[0]

# Plot Impulse Response Functions (IRFs) using Cholesky decomposition
irf = results.irf(12)
irf.plot(impulse=shock_variable, orth=True)
plt.show()
...
The IRFs suggest that a shock to uncertainty raises the share of EE transitions to replacement hires.
In other words, replacement hires fall more than EE transition.
This implies that firms choose to replace fewer quits than usual in response to uncertainty.
Whether these replacements are postponed or completely cancelled is something to tackle in future research.

We can conclude that firms' willingness to replacement hire falls in response to uncertainty.
This stands in contrast to how labor markets were modeled in the literature so far.
In future work of mine, I plan to adjust the theoretical modeling to rationalize such changes in replacement hiring.

:::{.callout-note}
## Conclusion
While much work remains to be done, the initial evidence seems promising.
The relationship between uncertainty shocks and changes in replacement hiring behavior looks solid.
Now, both a convincing theory as well as a battery of robustness checks are needed to confirm the findings.
:::
